# GenAI_ML
Repository of my projects in GenAI_ML space

## 1.  Python Programming

  1.1  Personal Expense Tracker
  
  1.2  Task Manager with User Authentication

## 2.  Applied Data Science with Python

  2.1  Marketing Campaigns
  
  2.2  Sales Analysis

## 3.  Machine Learning

  3.1  Creating Cohorts of Songs
  
  3.2  Employee Turnover Analytics

## 4.  Deep Learning with Keras and TensorFlow

  4.1  Home Loan Prediction
  
  4.2  Lending Club Data Analysis

## 5.  Generative AI

  5.1  Crafting an AI Powered HR Assistant
  
  5.2  Creating Designs by Leveraging OpenAI and Gradio UI

## 6. Capstone Projects

  6.1  Autonomous Driving - Parts 1 & 2
  
  6.2  Preserving Heritage Enhancing Tourism with AI - Parts 1 & 2
  
  6.3  Sales Forecasting
